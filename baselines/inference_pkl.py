...
def merge(data_root, prefix):
    ...
    for file in os.listdir(data_root):
        if file.startswith(prefix):
            ...
            ori_data_lst.append(result[0].detach().cpu())
            adv_data_lst.append(result[1].detach().cpu())
            label_lst.append(result[2].detach().cpu())
    all_ori_pc = torch.concat(ori_data_lst, dim=0)  # [num_data, K, 3]
    all_adv_pc = torch.concat(adv_data_lst, dim=0)  # [num_data, K, 3]
    all_real_lbl = torch.concat(label_lst, dim=0)  # [num_data]
    results.append(all_ori_pc)
    results.append(all_adv_pc)
    results.append(all_real_lbl)
...
def generate_mini_batches(data, data2, data3, k):
    B = data.size(0)  # 获取数据的总数
    num_batches = B // k  # 计算可以生成的小批量数据的数量
    remainder = B % k  # 计算剩余的数据个数

    mini_batches = []
    mini_batches2 = []
    mini_batches3 = []
    for i in range(num_batches):
        mini_batch = data[i*k : (i+1)*k]  # 切片生成小批量数据
        mini_batch2 = data2[i*k : (i+1)*k]
        mini_batch3 = data3[i*k : (i+1)*k]
        mini_batches.append(mini_batch)
        mini_batches2.append(mini_batch2)
        mini_batches3.append(mini_batch3)

    if remainder > 0:
        mini_batch = data[-remainder:]  # 取出剩余的数据
        mini_batches.append(mini_batch)
        mini_batch2 = data2[-remainder:]  # 取出剩余的数据
        mini_batches2.append(mini_batch2)
        mini_batch3 = data3[-remainder:]  # 取出剩余的数据
        mini_batches3.append(mini_batch3)

    return mini_batches, mini_batches2, mini_batches3



def test_normal(data_path):
    """Normal test mode.
    Test on all data.
    """
    model.eval()
    ...
    with torch.no_grad():
        ...
        ori_data, adv_data, label = result[0], result[1], result[2]
        mini_batches, mini_batches2, mini_batches3 = generate_mini_batches(ori_data, adv_data, label, args.batch_size)
        for i in range(len(mini_batches)):
            ori_data, adv_data, label = \
                mini_batches[i].float().cuda(), mini_batches2[i].float().cuda(), mini_batches3[i].long().cuda()
            # to [B, 3, N] point cloud
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prefix', type=str,
                        default='UPerturb-pointconv-0.18-success')
