...
from baselines.util.augmentation import rotate_point_cloud, jitter_point_cloud, random_point_dropout, \
...
class ModelNetDataLoader(Dataset):
    def __init__(self, root, args, split='train', process_data=False):
        ...
        self.sharply_path = args.sharply_root
        ...
        p_root = 'baselines/official_data/modelnet40_normal_resampled'
        if self.uniform:
        ...
        if self.split == 'test' and self.sharply_path is not None:
            self.sharply_value = np.load(self.sharply_path, allow_pickle=True)['arr_0']
    def __len__(self):
    ...
    def _get_item(self, index):
        if self.process_data:
            ...
            if self.split == 'test'  and self.sharply_path is not None:
                sharply = self.sharply_value[index]
        else:
            fn = self.datapath[index]
            if self.split == 'test'  and self.sharply_path is not None:
                sharply = self.sharply_value[index]
        ...
        if self.split == 'test' and self.sharply_path is not None:
            return point_set, normal, label[0], sharply
        else:
            return point_set, normal, label[0]
...
if __name__ == '__main__':
    ...
    print(os.path.abspath('baselines/official_data/modelnet40_normal_resampled/'))
    data = ModelNetDataLoader('baselines/official_data/modelnet40_normal_resampled/', args, split='test', process_data=args.process_data)
    DataLoader = torch.utils.data.DataLoader(data, batch_size=12, shuffle=True)
    for point, normal, label,_ in DataLoader:
        print(point.shape)
        print(normal.shape)
        print(label.shape)