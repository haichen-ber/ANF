...
from baselines.attack import CWsharply_feature
...
def attack():
    ...
    for pc, normal, label, sharply in tqdm(test_loader):
        ...
        # attack!
        best_pc, success_num = attacker.attack(pc, normal, label, sharply)

        # results
        num += success_num
        all_ori_pc.append(pc)
        all_adv_pc.append(best_pc)
        all_real_lbl.append(label)

    # accumulate results
    all_ori_pc = torch.concat(all_ori_pc, dim=0)  # [num_data, K, 3]
    all_adv_pc = torch.concat(all_adv_pc, dim=0)  # [num_data, K, 3]
    all_real_lbl = torch.concat(all_real_lbl, dim=0)  # [num_data]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sharply_root', type=str,
                        default='sharply_value/pointnet/pointnet_feature1_64players-concat.npz')
    parser.add_argument('--batch_size', type=int, default=2, metavar='BS',
                        help='Size of batch')
    parser.add_argument('--budget', type=float, default=0.45, help='clip budget')
    parser.add_argument('--com_noise', default=True)
    parser.add_argument('--pos_noise', default=True)
    parser.add_argument('--neg_noise', default=True)
    ...
    parser.add_argument('--players', type=int, default=64, metavar='N',
                        help='num of players')
    parser.add_argument('--k_sharp', type=int, default=54, metavar='N',   ##25-18 64-54  44-34 15-10
                        help='num of k_sharp') ### 54 44 34 24 14 4
    ...
    parser.add_argument('--trans_weight', type=float, default=0.5, help='') ### 0.1 0.3 0.5 0.7 0.9
    ...
    attacker = CWsharply_feature(model_name=args.model.lower(), model=model,  
                                 adv_func=adv_func, com_noise=args.com_noise, pos_noise=args.pos_noise,
                                neg_noise=args.neg_noise, dist_func=dist_func,players=args.players, 
    ...
    # save results
    save_path = './attack/results/{}_feature1/sharply_feature_attack'.\
        format(args.model.lower())
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    save_name = 'Usharply_feature_attack-{}-{}-success_{:.4f}-rank_{}.npz'.\
        format(args.model, args.budget,
               success_rate, args.local_rank)
    np.savez(os.path.join(save_path, save_name),
            ori_pc=ori_data.detach().cpu().numpy().astype(np.float32),
            test_pc=attacked_data.detach().cpu().numpy().astype(np.float32),
            test_label=real_label.detach().cpu().numpy().astype(np.uint8))
