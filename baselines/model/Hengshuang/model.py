...
from baselines.model.Hengshuang.pointnet_util import PointNetFeaturePropagation, PointNetSetAbstraction
from baselines.model.Hengshuang.transformer import TransformerBlock
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        xyz = x[..., :3]
        points = self.transformer1(xyz, self.fc1(x))[0]  #self.fc1(x):[b,n,32]

        xyz_and_feats = [(xyz, points)] #将点云坐标xyz和点云特征points打包成一个元组并存储在列表xyz_and_feats中
        for i in range(self.nblocks):
            xyz, points = self.transition_downs[i](xyz, points) #xyz:[b,1024,3]->[b,256,3]->[b,64,3]->[b,16,3]->[b,4,3] points:[b,1024,32]->[b,256,64]->[b,64,128]->[b,16,256]->[b,4,512]
            points = self.transformers[i](xyz, points)[0] #[b,256,64]
...
class PointTransformerCls(nn.Module):
    ...
    def forward(self, x):
        points, _ = self.backbone(x) #特征维度:[b,4,512]
        res = self.fc2(points.mean(1)) #points.mean(1):[b,512]    res:[b,40]
        return res, [points.mean(1)]  #points[b,4,512]
