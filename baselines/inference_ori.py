...
sys.path.append('../')
sys.path.append('./')
from baselines.dataset import ModelNet40Attack, ModelNet40, ModelNetDataLoader, ModelNet40Normal,ScanObjectNN
...
def test_normal():
    ...
    num, denom = 0, 0
    with torch.no_grad():
        for data in test_loader_ori:
            data_ori,_, label_ori,_ = data
            label = label_ori.long().cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sharply_root', type=str, default='sharply_value/pointnet2/pointnet2_64players-concat.npz')
    ...
    if args.mode == 'target':
        ...
        test_set_ori = ModelNetDataLoader(root='baselines/official_data/modelnet40_normal_resampled', args=args, split='test', process_data=args.process_data)
